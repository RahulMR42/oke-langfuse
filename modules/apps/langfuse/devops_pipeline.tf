...
resource "null_resource" "langfuse_version" {
  provisioner "local-exec" {
    command = "echo $(git ls-remote --tags https://github.com/langfuse/langfuse | sort -t/ -k3V | tail -1 | awk -F '/' '{print $3}') > ${path.module}/langfuse.version"
  }
}
    ...
    dynamic "items" {
      for_each = nonsensitive([for i in [
        ...
        },
        {
            ...
            default_value = local.object_storage_bucket
        }
